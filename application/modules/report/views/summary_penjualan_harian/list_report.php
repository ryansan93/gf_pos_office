	...
	<?php foreach ($data as $k_data => $v_data): ?>
		<?php if ( (!isset($v_data['diskon_requirement']['OC']) || $v_data['diskon_requirement']['OC'] == 0) && (!isset($v_data['diskon_requirement']['ENTERTAIN']) || $v_data['diskon_requirement']['ENTERTAIN'] == 0) && isset($v_data['status_gabungan']) ) : ?>
			...
			<tr class="cursor-p" style="background-color: <?php echo $bg_color; ?>;">
				...
				<td class="text-right"><?php echo isset($v_data['diskon_requirement']['FOOD_PROMO']) ? angkaRibuan($v_data['diskon_requirement']['FOOD_PROMO']) : 0; ?></td>
				<!-- <td class="text-right"><?php echo isset($v_data['diskon'][2]) ? angkaRibuan($v_data['diskon'][2]) : 0; ?></td> -->
				<td class="text-right"><?php echo ((!isset($v_data['diskon_requirement']['FOOD_PROMO']) && $v_data['diskon_requirement']['FOOD_PROMO'] == 0) && isset($v_data['diskon'][1])) ? angkaRibuan($v_data['diskon'][1]) : 0; ?></td>
			...
			<?php
				...
				$tot6 += ((!isset($v_data['diskon_requirement']['FOOD_PROMO']) && $v_data['diskon_requirement']['FOOD_PROMO'] == 0) && isset($v_data['diskon'][1])) ? ($v_data['diskon'][1]) : 0;
