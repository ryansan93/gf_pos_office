...
class AdjustmentOut extends Public_Controller {
    ...
    public function getLists()
    {
        ...
        $d_adjout = $m_adjout->whereBetween('tgl_adjout', [$start_date, $end_date])->whereIn('branch_kode', $params['branch_kode'])->with(['branch'])->orderBy('kode_adjout', 'desc')->get();
    ...
    {
        $m_adjout = new \Model\Storage\Adjout_model();
        $d_adjout = $m_adjout->where('kode_adjout', $kode)->with(['branch', 'detail', 'log_tables'])->first();
    ...
    public function save()
    {
        ...
        try {
            ...
            $m_adjout->branch_kode = $params['branch'];
