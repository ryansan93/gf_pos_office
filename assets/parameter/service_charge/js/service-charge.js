var sc = {
	...
	modalAddForm: function () {
        ...
        $.get('parameter/ServiceCharge/modalAddForm',{
        },function(data){
            var _options = {
                className : 'large',
                message : data,
	...
	save: function () {
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data Service Charge ?', function (result) {
				if ( result ) {
					var params = {
					...
					$.ajax({
			            url: 'parameter/ServiceCharge/save',
			            data: {
			                'params': params
			            },
			            type: 'POST',
	...
	delete: function (elm) {
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data Service Charge ?', function (result) {
			if ( result ) {
				$.ajax({
		            url: 'parameter/ServiceCharge/delete',
		            data: {
		                'id_sc': $(elm).attr('data-id')
		            },
		            type: 'POST',
		            dataType: 'JSON',
		            beforeSend: function() { showLoading(); },
